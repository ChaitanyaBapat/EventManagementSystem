				...
				if ( $result->num_rows > 0 ) {
					...
					while( $row = $result->fetch_assoc () ) {
						...
						if( isset( $_SESSION['admin_login'] ) && $_SESSION['admin_login'] ) {
							...
							echo '<form action="../events/update_event.php" method="POST">';

							echo '<input type="hidden" name="e_id" value="'.$row['e_id'].'"></input>';

							echo '<input class="w3-container w3-button w3-highway-blue" style="position:absolute; right:0; bottom:0; display:block; width:49%; font-size:16px; padding-top:8px;padding-bottom:8px; box shadow:5px 5px 5px solid #888888" type="submit" value="Update Event"></input></form>';

							echo '<form action="../events/get_log.php" method="POST">';

							echo '<input type="hidden" name="e_id" value="'.$row['e_id'].'"></input>';

							echo '<input class="w3-container w3-button w3-highway-blue" style="position:absolute; left:0; bottom:0; display:block; width:49%; font-size:16px; padding-top:8px;padding-bottom:8px; box shadow:5px 5px 5px solid #888888" type="submit" value="Get Log"></input></form></a></div>';
						}
						else {
